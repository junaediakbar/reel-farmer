...
- **Review & seleksi klip** (`RunDetail.tsx`) — user memilih klip mana yang dirender (tidak semua kandidat otomatis diproses), bisa menambah klip custom manual atau import daftar klip lewat JSON (upload file atau paste). Sebelum "Export Selected", user memilih/menyesuaikan caption style lewat panel "Caption Style" (`CaptionStyleControls.tsx`) dengan **live preview** overlay di atas video sumber ter-trim (`CaptionOverlayPreview.tsx` + `SourceVideoPlayer.tsx`) — lihat G15 (selesai).
...
- **Editor caption** (`CaptionEditor.tsx`) — edit teks & timing tiap kata (drag untuk resize durasi, klik untuk seek), pilih dari 3 preset style (Pop/Minimal/Elegant) atau atur manual (font, size, weight, line spacing, warna, outline, animasi), **live preview** overlay ter-render di atas video klip (`desilenced.mp4`, sebelum caption dibakar) lewat `CaptionOverlayPreview.tsx`/`CaptionStyleControls.tsx` yang dipakai bareng dengan panel pre-export di `RunDetail.tsx`, retranscribe klip dalam bahasa lain, lalu re-render overlay saja (`regenerateCaptions`) tanpa mengulang seluruh pipeline.
...
| G15 _(revisi — selesai)_ | **Caption editor punya live preview, tapi hanya post-render** — root masalahnya sudah teratasi: overlay-rendering (`CaptionOverlayPreview.tsx`) dan style controls (`CaptionStyleControls.tsx`) diekstrak dari `CaptionEditor.tsx` jadi komponen bersama, lalu dipasang juga di `RunDetail.tsx` sebelum "Export Selected" (panel "Caption Style" + overlay di atas `SourceVideoPlayer`). Preview pre-export pakai baris contoh statis, bukan transkrip nyata (belum ada sebelum `GENERATE_CAPTIONS`) — jadi menunjukkan font/warna/posisi/animasi, bukan sinkronisasi kata sungguhan. | Rendah — sisa gap murni kosmetik (sample text, bukan transkrip) | Selesai (§5 item 24) |
...
| G18 _(selesai)_ | **"Generate More Clips" + undo/redo** — **selesai** (belum commit): `POST /api/runs/:id/generate-more` (`generateMoreClips` di `orchestrator.ts`) re-run `IDENTIFY_CLIPS` dan menambahkan kandidat baru ke `clips.json` tanpa menghapus yang lama, mengakumulasi `tokenUsage` di stage result yang sama; endpoint sinkron (bukan background+poll seperti `runUntilSelection`) karena ini satu panggilan AI, bukan seluruh pipeline — pola yang sama dipakai `regenerate`/`retranscribe` caption, jadi `idleTimeout` server dinaikkan ke 120s (default 10s Bun terlalu pendek untuk retry+backoff DeepSeek). Undo/redo di `RunDetail.tsx` pakai reducer murni `undoableReducer` (diuji lewat unit test tanpa rendering harness) yang membungkus seluruh `editableClips` (trim, judul, toggle pilih, custom clip, import JSON, generate-more). Ceiling yang diketahui: undo pada "Remove" mengembalikan metadata kandidat tapi bukan file render yang sudah dihapus (`server.ts` DELETE menghapus mp4/thumbnail tanpa syarat) — lihat komentar `ponytail:` di `removeClip`. | Rendah, tapi UX blocking kalau AI hasilkan <5 kandidat bagus | Selesai (§5 item 22) |
...
| 24  | Pindahkan live preview caption (dari #11) ke **sebelum** "Export Selected", bukan hanya post-render — user melihat style yang dipilih di atas video ter-trim sebelum render pertama jalan — **selesai**: overlay-rendering & style-controls JSX diekstrak dari `CaptionEditor.tsx` jadi `CaptionOverlayPreview.tsx`/`CaptionStyleControls.tsx` (dipakai bareng oleh editor post-render & panel baru), `SourceVideoPlayer.tsx` dapat slot `overlay`/`onTimeUpdate`, panel "Caption Style" collapsible dipasang di `RunDetail.tsx` sebelum tombol "Export Selected". Preview pakai baris contoh statis (bukan transkrip asli — itu belum ada sebelum `GENERATE_CAPTIONS` jalan), jadi hanya menutup gap font/warna/posisi/animasi, bukan sinkronisasi kata nyata. | G15      | S–M    |
...
| 22  | ~~Endpoint "Generate More Clips" pada run yang sudah selesai + undo/redo di editor klip~~ — **selesai**, lihat G18 (§4). | G18 | S–M    |
